...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 20px; padding: 2rem; transition: all 0.3s; position: relative; overflow: hidden; animation: float 6s ease-in-out infinite; }
...
export default function Home() {
  ...
  const features = [
    ...
    { icon: "📝", title: "Notes Generator", desc: "Generate exam-ready notes instantly. Protected view – copying or screenshots disabled.", protected: true },
  ];
          ...
          <div className="cards-grid">
            {features.map((f, i) => (
              <div
                className="card"
                key={i}
                onContextMenu={(e) => f.protected && e.preventDefault()}
                style={f.protected ? { userSelect: "none" } : {}}
              >
