...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 20px; padding: 2rem; transition: all 0.3s; position: relative; overflow: hidden; animation: float 6s ease-in-out infinite; user-select: none; }
  ...
  /* Notes generator specific */
  .notes-card { user-select: none; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; position: relative; }
  .notes-card::after { content: 'PAID CONTENT - DO NOT SHARE'; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-30deg); font-size: 1.2rem; color: rgba(255,0,0,0.2); font-weight: 700; pointer-events: none; z-index: 10; }
`;
...
export default function Home() {
  ...
  const features = [
    ...
    { icon: "📝", title: "Notes Generator", desc: "Generate concise, exam-ready notes. Paid content is protected and view-only." },
      ...
      <div className="noise">
        ...
        <section className="features" id="features">
          ...
          <div className="cards-grid">
            {features.map((f, i) => (
              <div className={`card ${f.title==="Notes Generator" ? "notes-card" : ""}`} key={i}>
        ...
        <section className="cta-section">
          ...
          <a href="#" className="btn-primary">Start for Free</a>
                ...
                <footer>
          © {new Date().getFullYear()} MAKAUT Helper. All rights reserved.
        </footer>
      </div>
    </>
  );
